...
| **Trade-offs** | Host-model-reads is **not deterministic** — two runs may differ. Accepted, bounded three ways: the kernel already refuses invented tokens (`BAD_TOKEN`, `registry-store.ts:167`); `learn.md` §3c already mandates SOURCE-grade with an "unverified" list; and 0/3 CVA means determinism is not purchasable at any honest price today. Revisit when the corpus shows a standard worth parsing. |
