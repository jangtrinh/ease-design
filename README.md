...
| Date | Change | Commit |
|---|---|---|
| 2026-07-22 | **Onboarding: capability-aware optional Figma track** — `ui onboard` figma-step hint now branches on whether `figma-agent` is on PATH (studio-clone add-on, not in the npm package): open-plugin + `figma-agent status` vs. clone the repo and run `./setup.sh`, never a dead `command not found`; step stays optional and never blocks `ready`. New `capabilities.figmaAgent` flag in `--json`; pure `figmaAgentOnPath` probe | `52149fc` |
| 2026-07-22 | **Figma Agent response integrity + bounded inspection** — `inspect --depth N` (default 1) bounds the walker AND both async pre-passes (`childrenTruncated` marks cuts); `scan-node`/`mirror-verify` stay deep. New `E_RESPONSE_INTEGRITY` gate + stdout drain kill the "exit 0 + truncated JSON" hole (size is telemetry only). `status --timeout` bounds live enrichment (`enrichTimedOut`) | `e21e4f6` |
